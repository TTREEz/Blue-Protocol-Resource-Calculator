...
const Store = (() => {
    ...
    function normalizeRecipe(r) {
        ...
        const nr = {
            ...
            Ingredients: {},
            YieldOutcomes: typeof r.YieldOutcomes === 'object' && r.YieldOutcomes ? r.YieldOutcomes : null,
            // NEW: optional chances for min/max window
...
(() => {
    ...
    // Percentage inputs (optional)
    const rYminP = document.querySelector('#rYminP');
    const rYmaxP = document.querySelector('#rYmaxP');

    // Tools panel
    const toolsBox = document.querySelector('#toolsBox');
    const sampleYields = document.querySelector('#sampleYields');
    const statCount = document.querySelector('#statCount');
    const statMean = document.querySelector('#statMean');
    const statMin = document.querySelector('#statMin');
    const statMax = document.querySelector('#statMax');
    const statStd = document.querySelector('#statStd');

    const btnApplyAvgFixed = document.querySelector('#btnApplyAvgFixed');
    const btnApplyRange = document.querySelector('#btnApplyRange');
    const btnClearSamples = document.querySelector('#btnClearSamples');
    const toolsMsg = document.querySelector('#toolsMsg');
    ...
    function loadForm(r) {
        ...
        // NEW: chances (store uses 0–1 internally; show as 0–100)
        const toPct = v => (v == null ? '' : (Number(v) <= 1 ? (Number(v) * 100) : Number(v)));
        rYminP.value = toPct(r?.YieldMinChance);
        rYmaxP.value = toPct(r?.YieldMaxChance);
    ...
    function readForm() {
        ...
        const rec = {
            ...
            // Optional chances; normalization will accept 0–1 or 0–100
            YieldMinChance: cleanPct(rYminP),
            YieldMaxChance: cleanPct(rYmaxP),

            Ingredients: {}
        };

        // gather ingredients (unchanged)
    ...
    function recomputeSamplesUI() {
        ...
        btnApplyAvgFixed.disabled = !(mean > 0); // keep fixed yield > 0
        btnApplyRange.disabled = false;
    }
    ...
    btnApplyAvgFixed?.addEventListener('click', e => {
        ...
        if (mean <= 0) {
            toolsMsg.textContent = 'Average is 0 — cannot set Fixed Yield to 0. Use “Set Min/Max” instead.';
            return;
        }
...
(() => {
    ...
    function effectiveYield(rec, mode) {
        ...
        // 2) Min/Max window (>= 0) with optional probabilities
        ...
        if (hasMin || hasMax) {
            ...
            // Average mode: use chances if given, else simple mean
            let pMin = rec.YieldMinChance, pMax = rec.YieldMaxChance;
            if (pMin == null && pMax == null) {
                return Math.max(0, (min + max) / 2);
            }
            if (pMin == null && pMax != null) pMin = 1 - pMax;
            if (pMax == null && pMin != null) pMax = 1 - pMin;
    ...
    function effectiveYield(rec, mode) {
        // 1) Probability outcomes if provided
        if (rec.YieldOutcomes && typeof rec.YieldOutcomes === 'object') {
            const pairs = Object.entries(rec.YieldOutcomes)
                .map(([k, v]) => [Number(k), Number(v)])
                .filter(([qty, p]) => Number.isFinite(qty) && qty > 0 && Number.isFinite(p) && p >= 0);
            if (pairs.length) {
                ...
                const sumP = pairs.reduce((s, [, p]) => s + p, 0) || 1;
                const avg = pairs.reduce((s, [q, p]) => s + q * (p / sumP), 0);
                if (mode === YieldMode.Safe) return Math.max(1, min);
                if (mode === YieldMode.Opt) return Math.max(1, max);
                return Math.max(1, avg);
            }
        }
        // 2) Min/Max window if present
        const hasMin = rec.YieldMin !== null && rec.YieldMin > 0;
        const hasMax = rec.YieldMax !== null && rec.YieldMax > 0;
        if (hasMin || hasMax) {
            const min = hasMin ? rec.YieldMin : rec.Yield;
            const max = hasMax ? rec.YieldMax : rec.Yield;
            if (mode === YieldMode.Safe) return Math.max(1, min);
            if (mode === YieldMode.Opt) return Math.max(1, max);
            return Math.max(1, (min + max) / 2);
        }
        // 3) Fixed yield
        return Math.max(1, rec.Yield || 1);
    }
